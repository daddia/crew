...
describe("@daddia/crew/webhooks", () => {
  ...
  it("GitLab verify rejects missing token header", () => {
    expect(() =>
      verifySignature("gitlab", Buffer.from("{}"), {}, "secret"),
  ...
  it("checkReplayWindow throws when timestamp is outside window", () => {
    ...
    expect(() =>
      checkReplayWindow({ timestampMs: ancient, windowMs: 60_000 }),
    ).toThrow(ReplayError);
  });
...
describe("@daddia/crew main entry", () => {
  it("does not expose webhook helpers on the core barrel", () => {
    ...
    ).not.toHaveProperty("verifySignature");
    expect(
      crew as Record<string, unknown>,
    ).not.toHaveProperty("createIdempotencyStore");
  });
});
