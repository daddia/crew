...
> **`packages/crew` exists as the single shared library. Both crews — `crews/delivery` and `crews/code-reviewer` — declare `@daddia/crew: workspace:*` as their only shared dependency and import all types, session utilities, hooks, loaders, and webhook primitives from it. The three legacy packages (`@daddia/contracts`, `@daddia/sdk`, `@daddia/webhooks`) are deleted. `pnpm build`, `pnpm typecheck`, `pnpm lint`, and `pnpm test` all pass from the repo root with no changes to any crew workflow, persona, or prompt.**
...
packages/crew/
  ...
  tsconfig.json
  src/
    agent.ts            # Agent, AgentCrew, AgentInput, AgentResult, AgentDefinition, PersonaName
    unit.ts
...
`crews/delivery` and `crews/code-reviewer` depend on `@daddia/crew`. Delivery
...
Introducing a synthetic crew-to-crew import (for example an `import` from
`crews/delivery` referencing `crews/code-reviewer`) causes `pnpm lint` to fail
with a `no-cross-crew-imports` violation. This confirms boundary enforcement
...
pnpm build          # exit 0; dist/ present for packages/crew, crews/delivery, crews/code-reviewer
pnpm typecheck      # exit 0; no new type errors in any package or crew
...
- **Single package or subpath exports?** Decided: one package (`@daddia/crew`)
  ...
  preserve the `better-sqlite3` boundary — `crews/code-reviewer` imports only
  from `"."` and does not pull in the native addon.

- **Which dep-cruiser rule governs the new package?** The existing
  `no-packages-importing-crews` rule already covers `packages/crew → crews/`
...
- `@daddia/crew` (`"."` and `"./webhooks"`) is the stable import contract for
  all current and future agent crews.
