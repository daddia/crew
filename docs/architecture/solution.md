...
3. **Agent SDK-agnostic.** Crew is a runtime and deployment layer on top of _an_ agent SDK, not a specific one. The SDK in use today resolves sessions, executes tools, and exposes a post-tool-use hook surface; any SDK that exposes that contract can be wired in without changing the runtime API. Crew's value lives above the SDK, not inside it. Contemporary durable-agent runtimes demonstrate the same contract on different stacks — evidence the boundary is real, and candidate substrates Crew can ride rather than reimplement.
...
Three layers sit _above_ the individual crew. Together they encode the "compound above the model" thesis from `strategy.md`:
...
[delivery-build]                   [delivery-qa]                  [delivery-review]
  poll Jira (To Do)                  poll Jira (In QA)               poll Jira (In Review)
       ...
       └─► transition: In QA              └─► transition: In Review        └─► transition: Done
            log workflow.handoff-to-qa         log workflow.handoff-to-review
            (Jira status is the handoff; downstream crews poll/webhook — §5.1)
```
...
| **Context control**                         | Always-on `prompt.md`; skills loaded progressively by description; context compaction before window overflow on long runs (Next). Untrusted author text fenced before inclusion in prompts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Human-in-the-loop gate (v1)**             | Blocking mid-run gates (clarification, PM stakeholder sign-off, remediation wait) use a `*-pending` story step, Jira as the human surface, and poller-driven resume with a crew-specific timeout env (`CLARIFICATION_TIMEOUT_HOURS`, `PM_REVIEW_TIMEOUT_HOURS`, `REMEDIATION_TIMEOUT_HOURS`). Timeout escalates to `Needs human review`. See [`delivery-review` flow contract](../design/crew-flows/delivery-review.md). Durable mid-run resume without polling graduates to `@daddia/crew/hitl` (§11) — not required for CREW-06.                                                                                                                                                                                                                        |
| **Eval quality (CrewBench)**                | Fixture-owned `evals/*.eval.ts` per crew; `crew eval` / `pnpm eval:ci` drives real sessions; gate vs soft assertions; `--strict` mode for release gating. Same surface production uses — a passing eval means the crew booted and met contract. Evals are not part of the CI workflow; they run locally or in a separate eval/release pipeline.                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **Observability**                           | Structured logs today; OTel traces + cost-per-run metrics planned (CREW-08). One boot log answers "what config is this running with?"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
the runtime contract is identical. CI (`.github/workflows/ci.yml`) runs
`pnpm format:check && lint && guard:invariants && typecheck && test && build`
on every PR and merge to `main`. Docker image builds and CrewBench evals are
intentionally deferred to a separate pipeline — not the CI workflow.
...
- **OTel tracing not yet wired.** Structured logs cover most needs; tracing
  closes when CREW-08 ships.
- **Docker and CrewBench not in CI.** Image builds (`docker build`) and
  `pnpm eval:ci` run locally or in a future eval/release pipeline; CI covers
  code quality only.
...
9. ~~**How is PM stakeholder sign-off detected in `delivery-review`?**~~
   **Resolved (CREW-06):** Jira comment containing `/pm-approve` (configurable via
   `PM_APPROVAL_COMMENT_PATTERN`) created after `stakeholder-review-pending`
   `started_at`, from an account in required `PM_APPROVER_ACCOUNT_IDS`. Poller
   resume mirrors `delivery-build` clarification-pending. See
   [`docs/work/06-delivery-review/design.md`](../work/06-delivery-review/design.md)
   §5.1–§5.2.
