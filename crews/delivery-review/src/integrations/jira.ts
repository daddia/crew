 ...
 *
 * These are NOT model calls. Agents use MCP for Jira access; this module is
 * for the workflow's own idempotent state transitions.
 */
...
/**
 * Returns the first PM approval comment matching pattern, approver allowlist,
 * and created-after timestamp, or undefined when none qualify.
 */
export function probePmApproval(
  ...
  return comments.find(
    (comment) =>
      approverIds.includes(comment.accountId) &&
      comment.body.includes(pattern) &&
      new Date(comment.created).getTime() > afterTimestamp,
  );
}
