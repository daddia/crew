...
const validPayload = JSON.stringify({
  id: 42,
  timestamp: Date.now(),
  transition: { transitionName: 'In QA' },
  issue: { key: 'CREW-99' },
});
...
describe('POST /webhooks/jira', () => {
  ...
  it('returns 403 when signature is invalid', async () => {
    ...
    expect(runQaWorkflow).not.toHaveBeenCalled();
  });

  it('dispatches runQaWorkflow for In QA transition', async () => {
  ...
  it('ignores non-In-QA transitions', async () => {
