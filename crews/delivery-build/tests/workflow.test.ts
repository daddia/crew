...
describe('runStory', () => {
  ...
  it('passes branchName to engineer during address-feedback', async () => {
    ...
    mockSeniorEngineer.mockImplementation(async (_input: AgentInput) => {
      ...
      return successResult({
        success: reviewCall > 1,
        artefacts: { branchName: 'feature/ENG-1-test', comments: ['fix x'] },
      });
    });
  ...
  it('polls pipeline after MR opens and proceeds when CI is green', async () => {
    ...
    expect(ctxBase.gitlab.getPipelineStatus).toHaveBeenCalledWith(
  ...
  it('passes context.ticket to engineer for the address-feedback task', async () => {
    ...
    expect(feedbackCall).toBeDefined();
    expect((feedbackCall![0] as AgentInput).context['ticket']).toMatchObject({
      summary: 'Test Story',
    });
  });
  ...
  it('passes sessionId from engineer.run() result to state.startStep() for the implement step', async () => {
    ...
    mockEngineer.mockResolvedValue(
      successResult({
        artefacts: { branchName: 'feature/ENG-1-test', title: 'Test', sessionId: 'sess_abc' },
      }),
    );
    mockSeniorEngineer.mockResolvedValue(successResult());
  ...
  it('passes sessionId from engineer.run() result to state.startStep() for the address-feedback step', async () => {
    ...
    mockEngineer
      ...
      .mockResolvedValueOnce(
        successResult({
          artefacts: { branchName: 'feature/ENG-1-test', title: 'Test', sessionId: 'sess_impl' },
        }),
      )
      .mockResolvedValueOnce(
        successResult({
          artefacts: { branchName: 'feature/ENG-1-test', title: 'Test', sessionId: 'sess_def' },
        }),
      );
    mockSeniorEngineer.mockImplementation(async (_input: AgentInput) => {
      reviewCall++;
    ...
    expect(vi.mocked(state.startStep)).toHaveBeenCalledWith(
      'ENG-1',
      'address-feedback',
      'sess_def',
    );
  ...
  it('proceeds to In Progress when engineer returns questionsRequired: false', async () => {
    ...
    mockEngineer.mockResolvedValue(
      successResult({
        artefacts: { questionsRequired: false, branchName: 'feature/ENG-1-test', title: 'Test' },
      }),
    );
    mockSeniorEngineer.mockResolvedValue(successResult());
  ...
  it('records clarification-pending step in state store when questions are required', async () => {
    ...
    const finishCall = vi
      .mocked(state.finishStep)
      .mock.calls.find((c) => c[1] === 'clarification-pending');
    expect(finishCall).toBeDefined();
  ...
  it('records startStep and finishStep for assess-clarification when ticket is clear', async () => {
    ...
    const finishCall = vi
      .mocked(state.finishStep)
      .mock.calls.find((c) => c[1] === 'assess-clarification');
    expect(finishCall).toBeDefined();
