...
function setPollerState(tickAt: number | null, tickStatus: 'ok' | 'error' | null): void {
  ...
  (pollerState as { lastTickAt: number | null; lastTickStatus: 'ok' | 'error' | null }).lastTickAt =
    tickAt;
  (
    pollerState as { lastTickAt: number | null; lastTickStatus: 'ok' | 'error' | null }
  ).lastTickStatus = tickStatus;
}
