...
Each persona must declare a minimal `allowedTools` list. A reviewer should have no write tools. A gatekeeper that only reads diffs and approves MRs should not have Jira write access. Use `buildAuditHook()` from `@org/sdk` to enforce the allowlist at runtime as a second layer.
